...
void Actor::update(const int& frame_span, vector<glm::vec3>& location_list, vector<vector<int>>& next_index_list, vector<int>& destination_list) {
	...
	while (this->log.size() > 8) { this->log.erase(this->log.begin()); }
}
...
void ofApp::draw() {
	...
	for (int g = 0; g < this->parent_actor_group.size(); g++) {
		...
		for (auto& actor : this->actor_group_list[g]) {
			...
			ofNoFill();
			for (int i = 1; i < actor->getLog().size(); i++) {

				ofSetColor(actor->getColor(), ofMap(i, 0, actor->getLog().size(), 0, 255));
				ofDrawLine(actor->getLog()[i - 1] + this->parent_actor_group[g]->getLog()[i - 1],
					actor->getLog()[i] + this->parent_actor_group[g]->getLog()[i]);

			}

			ofFill();
			ofSetColor(actor->getColor());
			ofDrawCircle(actor->getLocation() + this->parent_actor_group[g]->getLocation(), 3);
		}
	}
